...
import { IconPlus, IconSearch, IconShield } from '../components/icons.jsx'
      ...
      <div className="card">
        <table className="tbl"><thead><tr><th style={{ width: 46 }}>ID</th><th>Câu hỏi</th><th style={{ width: 220 }}>Phân loại</th><th style={{ width: 130 }}></th></tr></thead>
          <tbody>{rows.map((q) => (
            <tr key={q.id}>
              <td>#{q.id}</td>
              <td><div style={{ whiteSpace: 'pre-wrap' }}>{q.content}</div>{q.image_url && <div style={{ marginTop: 6 }}><img src={q.image_url} alt="" style={{ maxWidth: 220, borderRadius: 8 }} loading="lazy" /></div>}<div className="small muted" style={{ marginTop: 4 }}>ĐA: {q.correct_answer || '—'}</div></td>
              ...
              <td><div className="row"><button className="btn sm" onClick={() => openEdit(q)}>Sửa</button><button className="btn danger sm" onClick={() => removeQ(q)}>Xóa</button></div></td>
            </tr>
          ))}</tbody></table>
